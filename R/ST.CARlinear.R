ST.CARlinear <- function(formula, family, data=NULL,  trials=NULL, W.quants, burnin, n.sample, thin=1, prior.mean.beta=NULL, prior.var.beta=NULL, prior.mean.alpha=NULL, prior.var.alpha=NULL, prior.nu2=NULL, prior.tau2=NULL, rho.slo=NULL, rho.int=NULL, MALA=FALSE, verbose=TRUE, Wstar.eigen = NULL)
{
    ...
    {
        if(!is.null(trials)) stop("you do not need a trials arugment as a binomial model was not specified", call.=FALSE)
        model <- poisson.CARlinear(formula=formula, data=data,  W.quants=W.quants, burnin=burnin, n.sample=n.sample, thin=thin, prior.mean.beta=prior.mean.beta, prior.var.beta=prior.var.beta, prior.mean.alpha=prior.mean.alpha, prior.var.alpha=prior.var.alpha, prior.tau2=prior.tau2, rho.slo=rho.slo,  rho.int=rho.int, MALA=MALA, verbose=verbose, Wstar.eigen = Wstar.eigen)          
    }else
    {
